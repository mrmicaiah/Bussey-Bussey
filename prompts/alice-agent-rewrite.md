# Alice — system prompt + greeting rewrite (worker)

Replace the public chat assistant's identity, voice, and behavior. She is now **Alice** —
an engaging AI salesperson who is Bussey's front door. KEEP ALL TOOLS AND PLUMBING EXACTLY
AS-IS (same save_lead tool, capture-and-handoff, no booking, no new capabilities). This is
a CONTENT change to two things only:

1. `worker/src/config/chat-system-prompt.ts` — replace CHAT_SYSTEM_PROMPT with the new
   text below; bump CHAT_SYSTEM_PROMPT_VERSION to `v2.0.0-alice-2026-05-26`.
2. `worker/src/routes/public/chat.ts` — replace the INITIAL_GREETING constant with Alice's
   new greeting below.

Do NOT change: the save_lead tool, the runChatTurn logic, the session/message/DB plumbing,
the merge behavior, the notification logic, or anything else. Only the prompt string,
version constant, and greeting string.

After changes: the worker must be redeployed manually (`wrangler deploy --env staging`) —
this is a worker change, not a frontend push. Build/typecheck to confirm it compiles.
Commit, do not push.

---

## New INITIAL_GREETING (chat.ts)
...
## New CHAT_SYSTEM_PROMPT (chat-system-prompt.ts)

Replace the exported CHAT_SYSTEM_PROMPT with exactly this (keep the backtick template literal
format and the escaped \`save_lead\` backticks):

You are Alice — the AI assistant and front-door salesperson for Bussey and Bussey.
...
## Constraints
- ONLY change the two strings/version constant described. No tool, plumbing, or logic changes.
- Keep the template-literal format; escape the save_lead backticks as in the original file.
- Typecheck/build to confirm the worker compiles.
- Flag clearly in your report that the worker needs a manual `wrangler deploy --env staging`
  to take effect (the frontend push won't deploy worker changes).
- Commit, do not push.
