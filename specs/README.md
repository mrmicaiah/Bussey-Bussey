...
| `06-workflow-presentation-and-disposition.md` | Template presentation, demo embed, end-of-deck outcomes, two-window mode |
| `07-workflow-acceptance-and-activation.md` | Accepted disposition → project creation → account creation → credentials handoff |
...
- **Acceptance is what locks.** Draft proposals are freely editable. Acceptance freezes the snapshot, scope, and contract terms.
- **Three-tier editability post-acceptance:** scope/pricing (sacred, change-order-only), terms (sacred but amendable via signed amendment), admin data (always editable with audit logging).
- **Clone for fresh pricing.** Edits to a draft proposal keep the original rates. To get current rates, clone the proposal.
- **Template-driven everywhere.** Presentations, proposals, contracts, change orders — all templated with content swapped in.
- **Two-window presentation flow.** Admin window controls; presentation window displays. Live sync keeps them in step.
